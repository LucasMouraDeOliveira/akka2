...
/**
 * Acteur de l'application répartie, relié à d'autres noeuds par une liste de
 * voisins. A la réception d'un message valide ({@link IdMessage}), le noeud
 * vérifie l'identifiant du message, et si cet identifiant est inconnu (le
 * message n'a pas encore été traité), il le propage à ses voisins.
 * 
 * Les messages peuvent venir de deux sources : soit c'est un voisin qui l'a
 * envoyé (et dans ce cas le message n'est pas renvoyé à ce voisin là), soit
 * c'est un message venant d'un utilisateur (sans référence de sender), et le
 * message est transmis à tous les voisins.
 * 
